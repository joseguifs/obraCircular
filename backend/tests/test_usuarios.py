...
async def test_criar_usuario_nome_vazio_e_rejeitado(client: AsyncClient) -> None:
    resposta = await client.post(
        "/api/v1/users", json={**USUARIO_PADRAO, "nome": "   "}
    )
...
async def test_criar_usuario_telefone_invalido_e_rejeitado(client: AsyncClient) -> None:
    resposta = await client.post(
        "/api/v1/users", json={**USUARIO_PADRAO, "telefone": "123"}
    )
    assert resposta.status_code == 422
...
async def test_criar_usuario_status_invalido_e_rejeitado(client: AsyncClient) -> None:
    resposta = await client.post(
        "/api/v1/users", json={**USUARIO_PADRAO, "status": "REMOVIDO"}
    )
    assert resposta.status_code == 422
...
async def test_obter_usuario_por_id(client: AsyncClient) -> None:
    ...
    resposta = await client.get(f"/api/v1/users/{criado['id']}")
    assert resposta.status_code == 200
    assert resposta.json()["id"] == criado["id"]


async def test_obter_usuario_inexistente_retorna_404(client: AsyncClient) -> None:
    resposta = await client.get("/api/v1/users/00000000-0000-0000-0000-000000000000")
...
async def test_listar_usuarios_nao_inclui_excluidos(client: AsyncClient) -> None:
    ...
    resposta_delete = await client.delete(f"/api/v1/users/{excluido['id']}")
    assert resposta_delete.status_code == 204

    resposta_lista = await client.get("/api/v1/users")
...
async def test_usuario_excluido_nao_aparece_mais_no_get_por_id(client: AsyncClient) -> None:
    ...
    await client.delete(f"/api/v1/users/{criado['id']}")

    resposta = await client.get(f"/api/v1/users/{criado['id']}")
    assert resposta.status_code == 404


async def test_excluir_usuario_ja_excluido_retorna_404(client: AsyncClient) -> None:
    criado = await _criar_usuario(client)

    primeira = await client.delete(f"/api/v1/users/{criado['id']}")
    segunda = await client.delete(f"/api/v1/users/{criado['id']}")
...
async def test_atualizar_usuario_parcial(client: AsyncClient) -> None:
    ...
    resposta = await client.patch(
        ...
        json={"nome": "Ana Maria Silva", "status": "INATIVO"},
    )
...
async def test_atualizar_usuario_inexistente_retorna_404(client: AsyncClient) -> None:
    resposta = await client.patch(
